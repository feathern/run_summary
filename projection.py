...
def plot_ortho(data_in,costheta,fig, ax, cax, latcen=45, loncen=0, nyz=512, colormap="RdYlBu",
               ...
               zero_lat = None, colat = None, axis_label = '', ncart=0):
    """
    z
    |
    ...
    if (colat != None):
        lat = -180*theta/numpy.pi + 90
        codata = 0*data-1
        print('min/max1: ', numpy.min(theta), numpy.max(theta), numpy.min(lat), numpy.max(lat))

        for i in range(ntheta):
            if (lat[i] > colat):
                codata[:,i]=1
        print('min/max: ', numpy.min(codata), numpy.max(codata), colat)
    ...
    if (colat != None):
        y2,z2,d2 = transform_coordinates(theta2d,phi2d,alpha,data=codata)
    
    if (zero_lat != None):
        rmx = numpy.sin(zero_lat*numpy.pi/180)
        for i, yv in enumerate(y):
            r = numpy.sqrt(yv*yv+z[i]*z[i])
            if (colat != None and d2[i] > 0):
                r = 0
            if (r > rmx):
                data_extract[i] = 0
    ############################################################
    ...
    ######################################
    if (ncart > 0):
        xvals = numpy.linspace(-1,1,ncart,dtype='float64')
        yvals = numpy.linspace(-1,1,ncart,dtype='float64')
        for i in range(ncart):
            xv = xvals[i]*nyz/2+nyz/2
            yv = yvals[i]*nyz/2 +nyz/2
            mn = -1.01*nyz/2+nyz/2
            mx = 1.01*nyz/2 +nyz/2
            ax.plot([mn,mx], [yv,yv], color='black',linewidth=0.2)
            ax.plot([xv, xv], [mn,mx],color='black',linewidth=0.2)
    ...
    fig_aspect = ax_delta_x/ax_delta_y # assumes subplot aspect ratio is 1
        ...
        # Mollweide))
    cbar_width = 0.8*ax_delta_x # make cbar 80% as long as plot is wide
    ...
    # sizing.
    cbar = plt.colorbar(dshow, cax=caxi, orientation='horizontal', format='%.0e', ticks=[dmin,dmax])# , label='cm/s') # , shrink=0.15, aspect=0.1)
    cbar.ax.set_xlabel(axis_label)
