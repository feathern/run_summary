...
class Shell_Avgs:
    ...
    self.lut                           : Lookup table for the different diagnostics output
    """
    def __init__(self,filename='none',path='Shell_Avgs/',ntheta=0, time_avg=False):
        """filename  : The reference state file to read.
           path      : The directory where the file is located (if full path not in filename)
        """
        if (filename == 'none'):
...
class AZ_Avgs:
    ...
    self.lut                                      : Lookup table for the different diagnostics output
    ------------------------------------------------------------------------------------------------------
      If reading a time-averaged file, self.iters and self.time will have 2 elements each, corresponding
      to the first/last iter/time used in the average respectively.
    -------------------------------------------------------------------------------------------------------
    """

    def __init__(self,filename='none',path='AZ_Avgs/', time_avg = False):
        """filename  : The reference state file to read.
           path      : The directory where the file is located (if full path not in filename)
           time_avg  : Set to True if reading a time-averaged series of AZ_Avgs
        """
        if (filename == 'none'):
            the_file = path+'00000001'
        else:
...
class Shell_Spectra:
    ...
    def __init__(self,filename='none',path='Shell_Spectra/', time_avg = False):
        """
           filename  : The reference state file to read.
           path      : The directory where the file is located (if full path not in filename)
        """
...
def Compile_GlobalAverages(file_list,ofile, qcodes=[],nfiles=-1):
    ...
    ffirst = 0
    if (nfiles > 0):
        ffirst = num_files-nfiles
    if (ffirst < 0):
        ffirst = 0
    if (len(qcodes) == 0):
        nq = a.nq
        qvals = numpy.zeros(nq,dtype='int32')
        qvals[:] = a.qv[:]
    else:
        nq = len(qcodes)
        qvals = numpy.zeros(nq,dtype='int32')
    ...
    for i in range(ffirst, num_files):
        ...
        for j in range(nrec):
            ...
            if (time_check):        
                for q in range(nq):
                    qcheck = a.lut[qvals[q]]
                    if (qcheck < maxq ):
                        tmp[q] = a.vals[j,q]
                tmp.tofile(fd)
                iteration[0] = a.iters[j]
                simtime[0] = a.time[j]
                simtime.tofile(fd)
                iteration.tofile(fd)
                icount[0] = icount[0]+1
...
def TimeAvg_AZAverages(file_list,ofile,qcodes=[],dt=-1,nfiles=-1):
    ...
    #   qcodes -- optional -- quantity values to compile in the time average
    #                         (defaults to all quantity codes)
    num_files = len(file_list)
    flast = -1
    if (nfiles > 0):
    ...
    a = AZ_Avgs(file_list[-1], path = '')

    #if (diter > 0):
    #    last_iter = a.time[a.nrec-1]
    ...
    if (len(qcodes) == 0):
        ...
        qvals = numpy.zeros(nq,dtype='int32')
        qvals[:] = qcodes[:]
    time_avg = np.zeros((ntheta,nr,nq),dtype='float64')
    ...
    qcount = np.zeros(nq,dtype='int32')
    ...
    for i in range(num_files-1,flast,-1):
        ...
        btheta = b.ntheta
        br = b.nr
        # Take care not to average in data with different dimensions
        if ((btheta == ntheta) and (br == nr) ):
            for j in range(nrec):
                # Time and iter check
                ...
                if (time_check):
                    for q in range(nq):
                        qcheck = b.lut[qvals[q]]
                        if (qcheck < maxq):
                            time_avg[0:ntheta,0:nr,q] += b.vals[:,:,qcheck,j].astype('float64')
                            qcount[q]+=1
                    t0[0] = b.time[j]
                    i0[0] = b.iters[j]
...
def TimeAvg_ShellAverages(file_list,ofile,qcodes=[],dt=-1,nfiles=-1):
    ...
    a = Shell_Avgs(file_list[nfiles-1], path = '')
    nr = a.nr
    ...
    if (a.version == 1):
        time_avg = np.zeros((nr,nq),dtype='float64')
    else:
        time_avg = np.zeros((nr,4,nq),dtype='float64')        
            ...
            for j in range(nrec):
                ...
                if (time_check):
                    for q in range(nq):
                        ...
                        if (qcheck < maxq):
                            if (a.version == 1):
                                time_avg[0:nr,q] += b.vals[0:nr,qcheck,j].astype('float64')
                            else:
                                time_avg[0:nr,0:4,q] += b.vals[0:nr,0:4,qcheck,j].astype('float64')
                            qcount[q]+=1
                    t0[0] = b.time[j]
                    i0[0] = b.iters[j]
...
def TimeAvg_ShellSpectra(file_list,ofile,qcodes=[],dt=-1,nfiles=-1):
    #   nfiles -- optional -- number of files to read relative to last file 
    #                         in the list
    #   dt     -- optional -- maximum time to average over, relative to
    #                         last iteration of last file in the list
    ...
    time_avg = np.zeros((nell, nm, nr,nq),dtype='float64')
                ...
                if (time_check):
                    for q in range(nq):
                        ...
                        if (qcheck < maxq):
                            ...
                            time_avg[:,:,:,q] += np.real(b.vals[:,:,:,qcheck,j])**2
                            time_avg[:,:,:,q] += np.imag(b.vals[:,:,:,qcheck,j])**2
                            qcount[q]+=1
    ...
    ndim=6
    if (a.version < 4):
        a.version = 4 # Make sure that Shell Spectra doesn't accidentally divide by sqrt(2)
    ...
    test = np.transpose(np.sqrt(time_avg))  # Store the square root of the power
    test.tofile(fd)
    t0.tofile(fd)
    i0.tofile(fd)
    # The final structure is identical to a normal az_average file save for the fact that final iteration adn final time are saved
